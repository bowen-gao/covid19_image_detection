    ...
    for epoch in range(num_epochs):
        ...
        for k in range(5):
            for phase in ['train', 'val']:
                ...
                train_num = dataloaders["train_num"][k]
                val_num = dataloaders["val_num"][k]
                running_loss = 0.0
                running_corrects = 0
                tp = 0
                num_covid = 0
                ...
                # Iterate over data.
                for i_batch, sample_batched in enumerate(dataloaders[phase][k]):
                ...
                if phase == "train":
                    k_train_loss += epoch_loss
                    k_tarin_rec += epoch_recall
                else:
...
def main():
    ...
    dataloaders_dict = {"train": [], "val": [], "train_num": [], "val_num": []}
    for train_covid_index, val_covid_index in kf.split(covid_index):

        train_normal_index = np.random.choice(normal_index, len(normal_index) - len(val_covid_index), replace=False)
        train_pneumonia_index = np.random.choice(pneumonia_index, len(pneumonia_index) - len(val_covid_index),
                                                 replace=False)
        train_index = []
        ...
        dataloaders_dict['train'].append(train_loader)
        dataloaders_dict['val'].append(val_loader)
        dataloaders_dict['train_num'].append(train_num)
        dataloaders_dict['val_num'].append(val_num)
